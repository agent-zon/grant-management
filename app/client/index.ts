...
import {
    ...
    getServiceBinding,
    getUserToken,
    alwaysProvider, useOrFetchDestination, subscriberFirst
...
app.get("/destinations/:name", async (c) => {
        ...
        try {
            ...
            return c.json({destination, destWithJwt, dest
                // ,destFromServiceBindingNoJwt,destFromServiceBinding
            });
        } catch (e: any) {
            return c.json({destWithJwt, dest,
                // destFromServiceBindingNoJwt,destFromServiceBinding,  
...
app.get("/debug", async (c) => {
    ...
    const userToken = srvBinding && await getUserToken(srvBinding, c.req.header("Authorization") || "");
