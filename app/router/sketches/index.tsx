...
function loadSketchDestinations(): SketchDest[] {
  try {
    ...
    if (Array.isArray(data)) {
      return data
        .filter((d: { name?: string; url?: string; description?: string }) => d.name && d.url)
        .map((d: { name: string; url: string; description?: string }) => ({
          name: d.name,
          url: d.url,
          labels: [] as string[],
...
sketches.get("/:sketch/tile", (c) => {
  ...
  return c.html(<div class="bg-white border border-slate-200 rounded-xl p-6 shadow-sm">
    ...
    <p class="text-sm text-slate-500 truncate mb-3">{d.description || `/sketches/${sketch}`}</p>
    <a
      ...
      hx-push-url="true"
      hx-swap="innerHTML"
      class="text-sm text-sky-600 hover:underline cursor-pointer"
    >
      Open sketch →
    </a>
  </div>);
