Workgroup:OpenID AuthZENPublished:8 November 2025Authors:
O. Gazitt, Ed.
Aserto
...
By convention, we refer to a client of the Authorization API as a Policy Enforcement Point, or PEP. Clients may consume the Authorization API for use cases that go beyond enforcement of authorization decisions; for example, the Resource Search API (Section 8.5) allows a caller to discover the resources on which a subject can perform an action. For consistency, we use the term PEP to describe a client of the API, regardless of the use case.

The Authorization API is defined in a transport-agnostic manner. A normative HTTPS binding is described in Transport (Section 10). Other bindings, such as gRPC, may be defined in other profiles of this specification.
...
3. Features
   The core feature of the Authorization API is the Access Evaluation API (Section 6), which enables a PEP to determine whether a specific request can be permitted to access a specific resource. The following are non-normative examples:
...
The Access Evaluations API (Section 7) enables execution of multiple evaluations in a single request. The following are non-normative examples:

Can Alice view documents 123, 234 and 345 on Tuesday, June 11, 2024?

Can document 123 be viewed by Alice and Bob?

The Search APIs (Section 8) provide lists of resources, subjects or actions that would be allowed access. The following are non-normative examples:
...
Figure 9: Example Context with a reference to a JSON schema
5.5. Decision
...
Figure 13: Non-normative Example Response with a step-up request Context 6. Access Evaluation API
...
6.1. The Access Evaluation API Request
The Access Evaluation request is an object consisting of four entities previously defined in the Information Model (Section 5):
...
6.2. The Access Evaluation API Response
The response of the Access Evaluation API consists of the Decision entity as defined in the Information Model (Section 5).
...
7.1. The Access Evaluations API Request
The Access Evaluation API Request builds on the information model presented in Section 5 and the object defined in the Access Evaluation Request (Section 6.1).

To send multiple access evaluation requests in a single message, the PEP MAY add an evaluations key to the request. The evaluations key is an array which contains a list of objects, each typed as the object as defined in the Access Evaluation Request (Section 6.1), and specifying a discrete request.

If an evaluations array is NOT present or is empty, the Access Evaluations Request behaves in a backwards-compatible manner with the (single) Access Evaluation API Request (Section 6.1).
...
The top-level subject, action, resource, and context keys provide default values for their respective fields in the evaluations array. The top-level subject, action and resource keys MAY be omitted if the evaluations array is present, contains one or more objects, and every object in the evaluations array contains the respective top-level key. This behavior is described in Section 7.1.1.
...
To perform a search, the PEP provides the Subject, Resource, Action, and Context entities defined in the Information Model (Section 5), but omits the unique identifier of the entity being queried. The PDP then responds with the set of authorized entities for the queried entity type which would be authorized according to the provided criteria.
...
Additional keys MAY be included in the page object. If they are, they MUST be defined in a specification referenced in the AuthZEN Policy Decision Point Capabilities Registry (Section 12.3). Furthermore, the PDP MUST declare support for the corresponding capability URN in its supported_capabilities metadata (Section 9.1.2).
...
count:
OPTIONAL. A non-negative integer indicating the number of results included in this response. When included at the start of a response, as described in the Search API Response (Section 8.3), this enables a PEP to display a progress indicator when processing large or slow responses.
...
page:
OPTIONAL. An object providing pagination information, as defined in Paginated Responses (Section 8.2.2). It is RECOMMENDED that the page object be the first key in the response, as this allows a PEP to use the count value to display a progress indicator when processing large or slow responses.

context:
OPTIONAL. An object that can convey additional information that can be used by the PEP, similar to its function in the Access Evaluation Response (see Section 6.2).

results:
REQUIRED. An array containing zero or more entities, as defined in the Information Model (Section 5). It MUST contain only entities of the type being searched for (e.g., Subjects, Resources, or Actions).
...
8.4. Subject Search API
The Subject Search API returns all subjects of a given type that are permitted according to the provided Action (Section 5.3), Resource (Section 5.2), and Context (Section 5.4).
...
8.5. Resource Search API
The Resource Search API returns all resources of a given type that are permitted according to the provided Action (Section 5.3), Subject (Section 5.1), and Context (Section 5.4).
...
8.6. Action Search API
The Action Search API returns all actions that are permitted according to the provided Subject (Section 5.1), Resource (Section 5.2), and Context (Section 5.4).
...
9.1.3. Signature Parameter
In addition to JSON elements, metadata parameters MAY also be provided as a signed_metadata value, which is a JSON Web Token [RFC7519] that asserts metadata values about the PDP as a bundle. A set of metadata parameters that can be used in signed metadata as claims are defined in Section 9.1.1. The signed metadata MUST be digitally signed or MACed using JSON Web Signature [RFC7515] and MUST contain an iss (issuer) claim denoting the party attesting to the claims in the signed metadata.
...
9.2. Obtaining Policy Decision Point Metadata
PDPs supporting metadata MUST make a JSON document containing metadata as specified in the AuthZEN Policy Decision Point Metadata Registry (Section 12.1) available at a URL formed by inserting a well-known URI string between the host component and the path and/or query components, if any. The well-known URI string used is /.well-known/authzen-configuration.
...
A successful response MUST use the HTTP status code 200 and a Content-Type of application/json. Its body MUST be a JSON object that contains a set of metadata parameters as defined in the AuthZEN Policy Decision Point Metadata Registry (Section 12.1).
...
Requests MUST include a Content-Type header with the value application/json, and the request body for each endpoint MUST be a JSON object that conforms to the corresponding request structure, as defined in Table 1.

A successful response is an HTTPS response with a status code of 200 and a Content-Type of application/json. Its body is a JSON object that conforms to the corresponding response structure, as defined in Table 1.

The request URL MUST be the value of the corresponding endpoint parameter, as defined in Table 1, if it is provided in the Policy Decision Point metadata (Section 9.1.1). If the parameter is not provided, the URL SHOULD be formed by appending the default path, as defined in Table 1, to the PDP's base URL (which is the policy_decision_point value from the Policy Decision Point metadata, if available.
...
Integer:
Represented as a JSON number (Section 6 of [RFC8259]). Note the recommendation in Section 11.5 to not encode values that exceed IEEE 754 double-precision.

Boolean:
Represented as the JSON literals true or false (Section 3 of [RFC8259]).

If a required attribute in the information model is omitted, the server MUST return a "Bad Request" error, as defined in Section 10.1.2.
...
Table 2: HTTPS Error status codes
...
Figure 37: Example of an HTTPS Action Search Response 11. Security Considerations
11.1. Communication Integrity and Confidentiality
...
Specification Document(s):
Section 9.1.1 of [This Document]
...
Specification Document(s):
Section 9.1.3 of [This Document]

12.2. Well-Known URI Registry
This specification asks IANA to register the well-known URI defined in Section 9.2 in the IANA "Well-Known URIs" registry [IANA.well-known-uris].
...
Repository: "AuthZEN Policy Decision Point Capabilities" registry (Section 12.3 of [This Document])
