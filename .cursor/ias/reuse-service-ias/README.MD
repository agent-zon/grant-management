...
And you could check the log of the __reuse service__ in the provider subaccount. You will then see some details of the token claims passed to the reuse service, such as the followings. Check the implementation in [ProviderController.java](./service/src/main/java/sample/spring/security/ProviderController.java#L45)
```
