...
In the example above a destination is registered under the name "DESTINATION" using `registerDestination()`. After that, you can reference this destination by passing its name.
...
1. Request an access token for the destination service and a given tenant ID from the XSUAA.
2. Due to the subscription between provider and subscriber, the XSUAA is allowed to issue the token.
3. The token allows for calling the destination service on behalf of the given tenant. The tenant and service information is encoded in the access token.
4. Make a call to the destination service using the obtained access token.
5. The destination maintained in the given tenant is returned.
