...
const {PUT, GET, POST, DELETE, axios} = cds.test(
    import.meta.dirname + "/.."
);
...
describe("OAuth 2.0 Basic Authorization Flow", () => {
    describe("PAR → Authorize → Consent → Token", () => {
                ...
                authorization_details: [
                    {
                        ...
                        "transport": "sse",
                        "locations": [
                            "https://mcp.devops.example.com/analytics"
                        ],
                    }
                ],
        ...
        it("should exchange authorization code for tokens", async () => {
            ...
            expect(data.authorization_details).toEqual(expect.arrayContaining([expect.objectContaining({
                type: "mcp",
                server: "devops-mcp-server",
                transport: "sse",
                tools: {
    ...
    describe("Multiple Consents per Grant", () => {
        ...
        it("2nd: should query grant and see combined scopes", async () => {
            const {data} = await GET(`/oauth-server/Grants/${grantId}?$expand=authorization_details`, {
                headers: {Accept: "application/json"},
            });
            expect(data).toMatchObject({
                scope: "openid devops workspace.fs",
...
async function submitConsent({request_ID, ...data}: any) {
    return await PUT(`/oauth-server/AuthorizationRequests/${request_ID}/consent`, data, {
        maxRedirects: 0,
        headers: {Accept: "text/html"},
