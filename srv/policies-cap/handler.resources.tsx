...
export async function RESOURCES(this: any, req: cds.Request) {
  ...
  return sendHtml(
    req,
    list
      .map((r: { value: string; label: string }) =>
...
export async function RESOURCES_TOGGLE(this: any, req: cds.Request) {
  ...
  if (!agent || !resource) {
    return req.reject(
      400,
      "Missing agent or resource " + resource + " " +
        JSON.stringify({ agent, version, resources, resource, data: req.data }),
    );
  }
  const list = Array.isArray(resources) ? resources : [];
...
export async function RESOURCES_ENABLE(this: any, req: cds.Request) {
  ...
  if (!agentId || !resource) {
    return req.reject(400, "Missing agentId or resource");
  }
...
export async function RESOURCES_PANE(this: any, req: cds.Request) {
  ...
  function resourcesListContent(
    ...
    return resources.map((
      r: { name: string; displayName: string; refFile?: string; slug: string },
    ) => (
      <div
        key={r.name}
        hx-get={`${r.slug}/card`}
