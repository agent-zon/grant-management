...
/** GET .../resources/pane → lightweight list; each row loads its card via hx-get (data fetched on demand). */
  ...
  return render(
    ...
    >
      ...
      <div className="flex-1 min-h-0 space-y-3 overflow-y-auto pr-1">
        {resources.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-8 border border-dashed border-gray-200 rounded-xl text-gray-500 bg-gray-50/50">
            ...
            <p className="text-xs mt-0.5">Connect an resource to discover tools</p>
          </div>
        ) : (
          resources.map((r: { name: string; displayName: string; refFile?: string; slug: string }) => (
            <div
                hx-get={`${r.slug}/card`}
                hx-trigger="load"
                ...
                hx-params="v"
                hx-vals="js:{ }"
                className="transition-all fade-in fade-out "
               >
            </div> 
          ))
        )}
      ...
      <div className="pt-4 border-t border-gray-200">
        <button
          type="button"
          className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl bg-gray-100 hover:bg-gray-200 border border-gray-200 text-gray-800 text-sm font-medium transition-colors"
        >
