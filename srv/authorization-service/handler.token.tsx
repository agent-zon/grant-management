...
export default async function token(
    ...
    if (refresh_token != null && cds.requires.auth.credentials) {
        ...
        console.log("refresh_token request", req.data,
        ...
        console.debug("token-response", data, tokenResponse.headers);
        return data;
    ...
    if(grant_type === "urn:ietf:params:oauth:grant-type:token-exchange" && cds.requires.auth.credentials){
        ...
        const tokenResponse= await fetch(tokenUr.href,{
            ...
            body: new URLSearchParams({
                ...
                subject_token: req.data.subject_token ||req.user?.authInfo?.token.jwt || "",
    ...
    const request = await this.read(AuthorizationRequests, code) as AuthorizationRequest;
    if (!request) {
    ...
    const authorization_details = await cds.run(
        cds.ql.SELECT.from(AuthorizationDetails).where({consent_grant_id: request.grant_id})
    );
