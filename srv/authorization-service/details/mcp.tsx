...
import { MCPToolAuthorizationDetailRequest } from '#cds-models/grant/management';
import './types.d.ts';

export default function MCPAuthorizationDetail({ index, description, riskLevel, category, ...detail }: MCPToolAuthorizationDetailRequest & AuthorizationDetailProps) {
  return (
    <div
          ...
          {detail.server && (
            <div className="bg-gray-600/50 rounded p-2">
              ...
              <div className="text-sm text-white">
                <input title='Server' name={`authorization_details[${index}].server`} type='text' className="inline-block bg-purple-500/20 text-purple-300 px-2 py-1 rounded text-xs" value={`🖥️ ${detail.server}`} readOnly />
              </div>
            </div>
          )}
          {detail.transport && (
            <div className="bg-gray-600/50 rounded p-2">
              ...
              <div className="text-sm text-white">
                <input title='Transport'  name={`authorization_details[${index}].transport`} type='text' className="inline-block bg-blue-500/20 text-blue-300 px-2 py-1 rounded text-xs" value={`🔗 ${detail.transport}`} readOnly />
              </div>
            </div>
          )}
          {detail.locations && (
      ...
      {detail.tools && Object.keys(detail.tools).length > 0 && (
        <div className="mb-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {Object.entries(detail.tools).map(([toolName, permission]) => (
              <div key={toolName} className="flex items-center space-x-3 p-3 bg-gray-600/30 rounded">
                <input
                  type="checkbox"
