...
export async function LIST(
  ...
  console.log("🔧 Reading grants list:", data, req.data?.id);
  if (cds.context?.http?.req.accepts("html") && data && !req.data.id) {
    const consentRecords = (await this.read(Consents)) as ConsentRecord[];
    const authorization_details = await this.run(
  ...
  console.log("🔧 Grants list:", data, req.data.id);
...
///WORKAROUND: correct cds types

type ConsentRecord = Consent & {
  grant_id: string;
  scope: string | null;
  subject: string | null;
  actor: string | null;
...
type AuthorizationDetailRecord = AuthorizationDetail & {
